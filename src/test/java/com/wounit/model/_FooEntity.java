...
public abstract class _FooEntity extends EOGenericRecord {
	...
	FooEntity localInstance = (FooEntity) EOUtilities
	...
	FooEntity eo = (FooEntity) EOUtilities.createAndInsertInstance(
		editingContext, _FooEntity.ENTITY_NAME);
	return eo;
    }

    public static NSArray<FooEntity> fetchAllFooEntities(
    ...
    public static NSArray<FooEntity> fetchFooEntities(
	    EOEditingContext editingContext, EOQualifier qualifier,
	    NSArray<EOSortOrdering> sortOrderings) {
	EOFetchSpecification fetchSpec = new EOFetchSpecification(
		_FooEntity.ENTITY_NAME, qualifier, sortOrderings);
	fetchSpec.setIsDeep(true);
	NSArray<FooEntity> eoObjects = (NSArray<FooEntity>) editingContext
		.objectsWithFetchSpecification(fetchSpec);
	return eoObjects;
    }

    public static FooEntity fetchFooEntity(EOEditingContext editingContext,
	    String keyName, Object value) {
	return _FooEntity.fetchFooEntity(editingContext,
	...
	NSArray<FooEntity> eoObjects = _FooEntity.fetchFooEntities(
	...
	if (count == 0) {
	    ...
	    throw new IllegalStateException(
		    "There was more than one FooEntity that matched the qualifier '"
			    + qualifier + "'.");
	}
	return eoObject;
    }

    public static FooEntity fetchRequiredFooEntity(
	    EOEditingContext editingContext, String keyName, Object value) {
	return _FooEntity.fetchRequiredFooEntity(editingContext,
		new EOKeyValueQualifier(keyName,
			EOQualifier.QualifierOperatorEqual, value));
    }
    ...
    public static FooEntity fetchRequiredFooEntity(
	    EOEditingContext editingContext, EOQualifier qualifier) {
	FooEntity eoObject = _FooEntity.fetchFooEntity(editingContext,
		qualifier);
	if (eoObject == null) {
	    throw new NoSuchElementException(
		    "There was no FooEntity that matched the qualifier '"
			    + qualifier + "'.");
	}
	return eoObject;
    }

    public static FooEntity localInstanceIn(EOEditingContext editingContext,
	    FooEntity eo) {
	FooEntity localInstance = (eo == null) ? null : (FooEntity) EOUtilities
