...
public class TestSaveChangesMatcher {
	...
	Mockito.doThrow(new NSValidation.ValidationException("error")).when(mockEditingContext).saveChanges();

	matcher.matchesSafely(mockEditingContext);
	matcher.describeTo(mockDescription);

	assertThat(mockDescription.toString(), is("successfully saved editing context\n     but got: com.webobjects.foundation.NSValidation$ValidationException: \"error\""));
    }
	...
	Mockito.doThrow(new NSValidation.ValidationException("An unexpectedError")).when(mockEditingContext).saveChanges();

	matcher.matchesSafely(mockEditingContext);
	matcher.describeTo(mockDescription);

	assertThat(mockDescription.toString(), is("expecting exception other than \"An expected error\" while saving the editing context\n     but got: com.webobjects.foundation.NSValidation$ValidationException: \"An unexpectedError\""));
    }
	...
	Mockito.doThrow(new RuntimeException("an expected error")).when(mockEditingContext).saveChanges();
	...
	Mockito.doThrow(new RuntimeException("an unexpected error")).when(mockEditingContext).saveChanges();
	...
	Mockito.doThrow(new RuntimeException("unexpected error")).when(mockEditingContext).saveChanges();
